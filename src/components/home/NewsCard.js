...
const NewsCard = ({ newsData, index }) => {
    ...
    return (
        <div className="news-card-container">
            <img className="news-card-image" src={newsData?.urlToImage ? newsData?.urlToImage : NewsCardThumbnail} alt="news card image" />
            ...
            <div className="news-card-time-bookmark">
                <div className="news-card-published-time">{ theDate.toLocaleString()} </div>
                <span onClick={(e) => bookmarked(e, index)}>{!newsData?.isBookmark ? "Bookmark" : "Bookmarked"}</span>
            </div>
